...
struct Edge {
 ...
 private:
  // A default direction been defined by pair， where pair.first  --> .
  // pair.second
...
std::ostream& operator<<(std::ostream& os, const Edge<Ty>& edge) {
  os << "Node(" << edge.GetStartNode()->Name() << ") -----> Node("
     << edge.GetEndNode()->Name() << ")";
  return os;
}
...
struct Graph {
 public:
  ...
  void DumpNodes() {
    for (const auto& item : nodes_) LOG(INFO) << "Node: " << item->Name();
  }
 ...
 private:
  std::shared_ptr<CNode> get_and_delete_zero_indegree_node(
      NodeVec& nodes, InEdgeMap& in_edges) {
    for (const auto& node : nodes) {
      LOG(INFO) << "Current node: " << node->Name();
      if (in_edges.find(node.get()) == in_edges.end() ||
          in_edges[node.get()].empty()) {
        typename std::vector<std::shared_ptr<CNode>>::iterator it =
...
template <class CNode, class CEdge>
void Graph<CNode, CEdge>::AddEdge(std::shared_ptr<CNode>& start,
                                  std::shared_ptr<CNode>& end) {
...
template <typename Ty>
void DFS(Graph<Node<Ty>, Edge<Ty>>& graph, Node<Ty>* node,
         std::vector<Node<Ty>*>& node_vec) {
...
template <typename Ty>
void BFS(Graph<Node<Ty>, Edge<Ty>>& graph, Node<Ty>* node) {
  std::deque<Node<Ty>*> node_queue;
  std::set<Node<Ty>*> visited_node;
  node_queue.push_back(node);
  while (!node_queue.empty()) {
    auto val = node_queue.front();
    visited_node.insert(val);
    node_queue.pop_front();
    if (visited_node.find(val) == visited_node.end()) {
      visited_node.insert(val);
      LOG(INFO) << *val;
    }
    auto out_set = graph.GetOutNodes(val);
    if (!out_set.empty()) {
      for (auto& vertex : out_set) {
        node_queue.push_back(vertex);
      }
    }
  }
}
